...
// 得到ACK帧确认的包号范围
func parsePRAckFrame(r *bytes.Reader, ackDelayExponent uint8, _ protocol.VersionNumber) (*PRAckFrame, error) {
	var frame *PRAckFrame
...
func (f *PRAckFrame) Append(b []byte, _ protocol.VersionNumber) ([]byte, error) {
...
func (f *PRAckFrame) Length(_ protocol.VersionNumber) protocol.ByteCount {
...
func (f *PRAckFrame) numEncodableAckRanges() int {
...
func (f *PRAckFrame) encodeAckRange(i int) (uint64 /* gap */, uint64 /* length */) {
	if i == 0 {
...
func (f *PRAckFrame) HasMissingRanges() bool {
	return len(f.AckRanges) > 1
}

func (f *PRAckFrame) validateAckRanges() bool {
...
func (f *PRAckFrame) LargestAcked() protocol.PacketNumber {
	return f.AckRanges[0].Largest
}

// LowestAcked is the lowest acked packet number
func (f *PRAckFrame) LowestAcked() protocol.PacketNumber {
	return f.AckRanges[len(f.AckRanges)-1].Smallest
}

// AcksPacket determines if this ACK frame acks a certain packet number
func (f *PRAckFrame) AcksPacket(p protocol.PacketNumber) bool {
